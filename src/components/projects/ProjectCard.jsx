...
const ProjectCard = ({ project }) => {
  ...
  return (
    <>
      <div className="group relative bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-blue-500/20">
        <div className="relative h-48 bg-gradient-to-br from-blue-900/20 to-purple-900/20 overflow-hidden">
          ...
          <div className="absolute top-4 right-4 z-20 flex gap-2 flex-wrap">
            {project.category.map((cat, index) => (
              <span
                key={index}
                className="px-3 py-1 bg-blue-500/20 backdrop-blur-sm text-blue-300 text-xs rounded-full border border-blue-500/30"
              >
                {cat}
              </span>
            ))}
            {project.team && (
              <span className="px-3 py-1 bg-green-500/20 backdrop-blur-sm text-green-300 text-xs rounded-full border border-green-500/30 flex items-center gap-1">
                <Users className="w-3 h-3" />
                Team
              </span>
            )}
            {project.private && (
              <span className="px-3 py-1 bg-yellow-500/20 backdrop-blur-sm text-yellow-300 text-xs rounded-full border border-yellow-500/30">
                Private
              </span>
            )}
            {!project.deployed && (
              <span className="px-3 py-1 bg-red-500/20 backdrop-blur-sm text-red-300 text-xs rounded-full border border-red-500/30">
                Not Deployed
              </span>
            )}
          </div>
